...
export default function HomeHeroCarousel({ ctaText, ctaLink, stats }: HomeHeroCarouselProps) {
  ...
  return (
    <section className="relative aspect-[9/5] lg:aspect-video max-h-[900px] overflow-hidden border-b border-brand-border">
      {/* Slides */}
      {slides.map((sl, i) => (
      ...
      {slides.length > 1 && (
        <>
          <button onClick={prev} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 hover:bg-white border border-brand-border rounded-full flex items-center justify-center text-brand-navy transition-all backdrop-blur-sm z-10 shadow-sm">
            <ChevronLeft size={18} />
          </button>
          <button onClick={next} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 hover:bg-white border border-brand-border rounded-full flex items-center justify-center text-brand-navy transition-all backdrop-blur-sm z-10 shadow-sm">
