...
export default function BatchCardsCarousel({ tracks }: BatchCardsCarouselProps) {
  ...
  return (
    <div className="w-full">
      {/* ─── Mobile View: Vertical Flex Stack (Ek ke niche ek) ─── */}
      <div className="flex flex-col gap-4.5 sm:hidden">
      ...
      {/* ─── Tablet & Desktop View: Smooth Horizontal Carousel ─── */}
      <div className="hidden sm:block relative group/carousel w-full">
        ...
        >
          {tracks.map((track, idx) => (
            <div
              key={idx}
              className="w-[340px] md:w-[360px] lg:w-[calc((100%-48px)/3)] shrink-0 snap-start h-full"
            >
              <BatchHoverCard track={track} />
            </div>
          ))}
