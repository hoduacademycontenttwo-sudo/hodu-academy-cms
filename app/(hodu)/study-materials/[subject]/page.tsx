...
const subjectData: Record<string, {
  ...
  'jee-main': {
    label: 'JEE Main Archive', icon: Atom,
    dbCategory: 'jee main',
  ...
  'jee-advanced': {
    label: 'JEE Advanced Master Series', icon: FlaskConical,
  ...
  'neet': {
    label: 'NEET High Yield Notes', icon: Dna,
    dbCategory: 'neet',
  ...
  'ncert-solutions': {
    label: 'NCERT Exemplar Solutions', icon: BookOpen,
    dbCategory: 'ncert',
  ...
  'cbse': {
    label: 'CBSE Board Practice Sets', icon: Target,
    dbCategory: 'cbse',
  ...
  'olympiad': {
    label: 'Olympiad Talent Series', icon: Award,
    dbCategory: 'olympiad',
...
async function resolveSubject(subject: string) {
  ...
  return {
    label: navLink.label, icon: BookOpen,
    ...
    <div className="space-y-0 animate-fade-in bg-white">
      ...
      <div className="bg-white border-b border-brand-border px-4 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-xs text-neutral-500 font-medium">
          ...
          <span>/</span>
          <span className="text-brand-maroon font-bold">{data.label}</span>
        </div>
      </div>
      ...
      <section className="bg-brand-maroon text-white py-16 sm:py-20 border-b border-brand-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
          <Link href="/study-materials" className="inline-flex items-center gap-1.5 text-white/80 hover:text-white text-xs font-bold transition-colors uppercase tracking-wider">
            <ArrowLeft className="h-3.5 w-3.5" /> All Materials
          </Link>
          <div className="flex items-start gap-4 pt-2">
            <div className="w-14 h-14 rounded-2xl bg-white text-brand-maroon flex items-center justify-center shrink-0 shadow-sm">
            ...
            <div>
              ...
              <p className="text-white/90 text-sm sm:text-base font-normal mt-2 max-w-xl leading-relaxed">{data.tagline}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white border-2 border-brand-border rounded-2xl p-8 shadow-xs">
              ...
              <h2 className="font-serif-editorial text-2xl font-bold text-neutral-900 mb-3">About This Exam Track</h2>
              <p className="text-xs sm:text-sm text-neutral-600 font-normal leading-relaxed">{data.overview}</p>
            </div>
            ...
            {data.syllabus.length > 0 && (
              <div className="bg-white border-2 border-brand-border rounded-2xl p-8 shadow-xs">
                ...
                <h2 className="font-serif-editorial text-2xl font-bold text-neutral-900 mb-4">Core Topics Covered</h2>
                <div className="space-y-3">
                  {data.syllabus.map((line, i) => (
                    <div key={i} className="flex items-start gap-3 bg-neutral-50 p-4 rounded-xl border border-brand-border">
                      <CheckCircle2 className="h-4 w-4 text-brand-maroon shrink-0 mt-0.5" />
                      <p className="text-xs font-semibold text-neutral-800">{line}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-white border-2 border-brand-border rounded-2xl p-8 shadow-xs">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-brand-maroon bg-neutral-100 px-3 py-1 rounded-md inline-block mb-2 border border-brand-border">
                    DOWNLOADABLE REPOSITORY
                  </span>
                  <h2 className="font-serif-editorial text-2xl font-bold text-neutral-900">Free PDF Problem Sets & Notes</h2>
                </div>
                {resources.length > 0 && (
                  <span className="text-xs font-bold text-brand-maroon bg-neutral-100 border border-brand-border px-3 py-1 rounded-full">
              ...
              {resources.length > 0 ? (
                <div className="space-y-3">
                  {resources.map((res) => (
                    <div key={res.id} className="flex items-center justify-between p-4 rounded-xl border border-brand-border hover:border-brand-maroon transition-colors group">
                      <div className="flex items-center gap-3.5">
                        <div className="w-10 h-10 rounded-lg bg-neutral-100 flex items-center justify-center shrink-0 border border-brand-border text-brand-maroon">
                          <FileText className="h-5 w-5" />
                        </div>
                        <div>
                          <p className="text-xs font-bold text-neutral-900 group-hover:text-brand-maroon transition-colors">{res.title}</p>
                          {res.content && <p className="text-[11px] text-neutral-500 mt-0.5 font-normal">{res.content}</p>}
                        </div>
                      </div>
                      <div className="flex items-center gap-2.5 shrink-0 ml-3">
                        <span className="text-[10px] font-bold border border-brand-border bg-neutral-100 text-neutral-800 px-2.5 py-0.5 rounded-md">{res.type}</span>
                        {res.file_url ? (
                          <a
                            href={res.file_url}
                            target="_blank"
                            rel="noreferrer"
                            className="flex items-center gap-1 text-xs text-brand-maroon font-bold hover:underline"
                          >
                ...
                <div className="text-center py-10 text-neutral-500">
                  ...
                  <p className="text-xs mt-1 font-normal">Submit your enquiry to receive curated PDF packs on WhatsApp.</p>
                </div>
              )}
            </div>
            ...
            {/* Tips */}
            {data.tips.length > 0 && (
              <div className="bg-brand-maroon text-white rounded-2xl p-8 sm:p-10 shadow-md">
                <span className="text-[10px] font-bold uppercase tracking-widest text-brand-maroon bg-white px-3 py-1 rounded-md inline-block mb-3 shadow-xs">
                  FACULTY DIRECTIVES
                </span>
                <h2 className="font-serif-editorial text-2xl sm:text-3xl font-bold text-white mb-6">Expert Preparation Rules</h2>
                <div className="space-y-3.5">
                  {data.tips.map((tip, i) => (
                    <div key={i} className="flex items-start gap-3.5 bg-white/10 border border-white/20 p-4 rounded-xl">
                      <span className="bg-white text-brand-maroon text-xs font-black w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 shadow">
                        {i + 1}
                      </span>
                      <p className="text-xs sm:text-sm text-white/90 font-normal leading-relaxed">{tip}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <aside className="space-y-6">
            <div className="bg-white border-2 border-brand-border rounded-2xl p-6 sm:p-7 shadow-md sticky top-28">
              ...
              <h3 className="font-serif-editorial font-bold text-neutral-900 text-lg mb-1">Get Direct WhatsApp PDFs</h3>
              <p className="text-xs text-neutral-500 mb-5 font-normal">Enter details to receive full answer keys and DPP sets.</p>
              <EnquiryForm />
            </div>

            <div className="bg-white border-2 border-brand-border rounded-2xl p-6 shadow-xs">
              <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-500 block mb-3">Other Subject Tracks</span>
              <div className="space-y-2">
                {otherSubjects.map(s => {
                  const SIcon = s.icon
                  return (
                    <Link key={s.slug} href={`/study-materials/${s.slug}`}
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-neutral-50 transition-colors text-xs text-neutral-900 hover:text-brand-maroon font-bold group border border-transparent hover:border-brand-border">
